...
除了上面的主站（9452），仓库还内置两套**独立阅读前端**（静态展示全部课文，公式完整渲染；判题练习与 Python 浮窗仍以主站为准，页面内有一键跳转）：

| 入口 | 双击 | 端口 | 风格 |
| --- | --- | --- | --- |
| 雅致版 | `启动FluentUI.bat` | http://localhost:9453 | 微软 Fluent / Windows 11：云母质感、亚克力侧栏、柔和光影 |
| 科幻版 | `启动科幻UI.bat` | http://localhost:9454 | FUI 全息 HUD：深空星野、扫描线、发光括角面板、可拖拽缩放的「星座图谱」先修网络 |

两套前端共用 `ui/` 目录下的自研渲染管线（`node ui/server.mjs`，零框架），互不干扰、也不影响主站。学习进度存在各端自己的浏览器本地存储（key：`ml-ui-progress`）。
