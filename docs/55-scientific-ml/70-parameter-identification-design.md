...
点越聚拢，直线越长袖善舞；两端一沉一浮挂上砝码，它就立刻老实。这份"稳与晃"的手感先记在心里——实验 2 马上把它翻译成一个可以计算的量。
...
实验 1 里"两端一拖、直线立刻老实"的手感，在扫描结果里现出了原形：行列式冠军（$(1,30)$，841）恰是力臂拉得最长的布点，垫底（相邻日，1）恰是挤成一团的布点——第 3 节那句 $\det=(t_2-t_1)^2$ 说的就是这回事，$\sqrt{841/1}=29$ 倍的标准差差距全部来自间距。
