...
3. 对比两次：库扩大了 **1000 倍**，样本需求只增加约 65%——$\ln$ 的缩放威力；而把 $\delta$ 从 0.05 收紧到 0.00005（同样千倍），分子虽只从 $\ln 40000=10.60$ 涨到 $\ln(4\times10^7)=17.50$，分母里的 $\delta^2$ 却要让样本需求暴涨约一百万倍——置信度按平方收费，"更自信"一点都不便宜。
...
- 完全不用变
? 余量项吃的是 ln(2M/δ)：M 乘 1000，ln 部分从 ln(2×1000/0.05)=ln 40000≈10.6 涨到 ln(2×10^6/0.05)=ln(4×10^7)≈17.5，需求比例约 17.5/10.6≈1.65，也就是六成五的上涨（样本量约从 2120 涨到 3501）——对数是容量党最好的朋友。
```
