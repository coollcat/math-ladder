---
title: 数列与归纳法 · 参考资料
description: 第 8 章涉及的核心论文、原著与延伸阅读一览。
---

# 数列与归纳法 · 参考资料

本章涉及的核心论文、原著与延伸阅读，按课程推进顺序整理。

文献页面对所有人开放；带归档副本的条目，未登录点「原站下载」前往出处，登录后点「本地下载」直接取本站副本。

```paper
# @title: Liber Quadratorum 与斐波那契数列
# @authors: Fibonacci（斐波那契）
# @year: 1225
# @venue: 数列与递推的经典案例
# @tag: 原著
# @desc: 兔子问题留下的数列，成为递推与归纳法最好的入门教材。
# @page: https://en.wikipedia.org/wiki/Fibonacci
# @pdf64: aHR0cHM6Ly9hcmNoaXZlLm9yZy9kb3dubG9hZC9qc3Rvci0yOTc0MDM5LzI5NzQwMzkucGRm
# @local64: L3BhcGVycy8yOTc0MDM5LWExMTYxMDFhLnBkZg==
# @lsize: 0.7 MB
```

```paper
# @title: Zeno's paradoxes（芝诺悖论）
# @authors: Zeno of Elea（芝诺）
# @year: 约公元前 450 年
# @venue: 无穷与极限的思想源流
# @tag: 原著
# @desc: 阿基里斯追不上乌龟？数列求和给出两千年后的答案。
# @page: https://en.wikipedia.org/wiki/Zeno%27s_paradoxes
```
