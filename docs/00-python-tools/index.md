...
在爬数学阶梯之前，先把登山杖检查一遍。这一章不教新数学——你上三件装备课：

1. [浮窗实验台与交互组件](./05-console-playground.md)——先认识本站的"实验室"：怎么写代码、怎么运行、怎么拖滑块、怎么让交互组件跟上代码；
2. [本站 Python 约定](./10-conventions.md)——把你会的基础语法对齐到"用代码研究数学"的用法上；顺便见证第一个工具 `sum()` 的诞生；
3. [matplotlib 入门](./20-matplotlib.md)——学会让数字变成图。

装备之外还有三张随翻随用的地图。**不必一次读完**：往后学卷一时忘了公式、忘了函数、忘了术语，就回来翻：

4. [公式速查表](./30-formula-sheet.md)——卷一所有公式的一行版索引，忘了就来翻；
5. [Python 工具索引](./40-python-index.md)——每个用过的函数记着用途和出生地；
6. [术语表](./50-glossary.md)——关键术语的中英对照一句话版。
