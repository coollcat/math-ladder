---
title: 微分几何与流形入门 · 参考资料
description: 第 57 章涉及的核心论文、原著与延伸阅读一览。
volume: 5
layer: L8
track:
  - geometry-space
  - analysis-change
stage: research-elective
difficulty: 4
---

# 微分几何与流形入门 · 参考资料

本章涉及的核心论文、原著与延伸阅读，按课程推进顺序整理。

「文献页面」与 PDF 下载对所有人开放；需要同步学习进度时可在「登录」页用默认账号进入自己的进度空间。

```paper
# @title: On the Hypotheses which lie at the Bases of Geometry
# @authors: Bernhard Riemann（黎曼）
# @year: 1854
# @venue: 哥廷根就职演讲
# @tag: 原著
# @desc: 「弯曲空间的度量由内蕴决定」：黎曼几何从此开始。
# @page: https://en.wikipedia.org/wiki/On_the_Hypotheses_which_lie_at_the_Bases_of_Geometry
```

```paper
# @title: Theorema Egregium（绝妙定理）
# @authors: Carl Friedrich Gauss（高斯）
# @year: 1827
# @venue: 曲面的弯曲不变量研究
# @tag: 论文
# @desc: 高斯曲率只由内蕴度量决定：生活在曲面上的生物也能测出曲率。
# @page: https://en.wikipedia.org/wiki/Theorema_Egregium
```
