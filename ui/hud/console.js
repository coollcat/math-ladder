/* ============================================================
   HUD 科幻皮肤 · 浮窗 Python 控制台(自包含,无外部依赖)
...
async function getPyodide(status) {
  ...
  pyodidePromise = (async () => {
    ...
    for (const base of CDNS) {
      try {
        status(`链接镜像 ${new URL(base).host} …`);
        await loadScript(base + 'pyodide.js', CDN_TIMEOUT_MS);
        status('初始化 Python 运行时…(首次约 10-30 秒)');
...
class PyConsole {
  constructor() {
    this.py = null;        // Pyodide 单例
    this.loading = false;  // 加载守卫
    this.running = false;  // 运行重入守卫
    this.mode = 'scratch'; // scratch | run | exercise
    this.exCtx = null;     // 判题上下文 { title, check, hint, payload }
    this.build();
  ...
  build() {
    ...
    fab.id = 'py-fab';
    fab.type = 'button';
    fab.title = 'Python 浮窗终端';
    fab.appendChild(el('i', 'fab-dot'));
    ...
    win.innerHTML = `
      <div class="py-title"><span class="py-mode" id="py-mode">随手算</span>
        <span id="py-title-text">PY·CONSOLE</span>
        <button type="button" class="py-x" id="py-close">ESC ✕</button></div>
      <div class="py-status is-idle" id="py-status">待命 // 首次运行将加载 Pyodide(约 20MB)</div>
      <div class="py-out" id="py-out"></div>
      <div class="py-diff" id="py-diff" hidden></div>
      <div class="py-in">
        <textarea id="py-code" spellcheck="false" placeholder="# 写点 Python,Ctrl+Enter 运行"></textarea>
        <div class="py-actions">
          <button type="button" class="hud-btn" id="py-run"><span>▶ 运行</span></button>
          <button type="button" class="hud-btn is-amber" id="py-clear"><span>清空输出</span></button>
  ...
  applyMode() {
    if (this.mode === 'exercise') {
      this.$mode.textContent = '判题中';
      this.$mode.classList.add('is-ex');
      this.$title.textContent = this.exCtx && this.exCtx.title ? 'EXERCISE // ' + this.exCtx.title : 'EXERCISE';
      this.$hintline.textContent = '目标输出见下方比对';
      if (this.exCtx && this.exCtx.hint) this.$hintline.textContent = '提示: ' + this.exCtx.hint;
    } else {
      this.$mode.textContent = '随手算';
      this.$mode.classList.remove('is-ex');
      this.$title.textContent = 'PY·CONSOLE';
      this.$hintline.textContent = '变量跨次保留';
    }
  }
  status(msg, cls) {
  ...
  async ensurePy() {
    ...
    try {
      ...
      this.fab.classList.add('is-live');
  ...
  async maybeLoadMatplotlib(code) {
    ...
    if (!this.py.runPython('import importlib.util; importlib.util.find_spec("matplotlib") is not None')) {
      this.status('装载 matplotlib 包…(首次较慢)', 'is-busy');
  ...
  async runConsole() {
    ...
    try {
      ...
      if (err) { this.print(err.replace(/\n$/, ''), 'po-err'); this.status('出错 —— 见红色输出', 'is-err'); }
      else this.status('完成 // ' + new Date().toTimeString().slice(0, 8), 'is-ready');
    } finally {
      // PyProxy 由 toJs 消费后无需手动销毁顶层结果
    }
  }
  ...
  async runExercise() {
    ...
    if (allOk && want.length > 0) {
      ...
      this.status('判题通过 // ' + new Date().toTimeString().slice(0, 8), 'is-ready');
      // 通过记录(djb2 哈希为 key)
      const rec = loadJSON('ml-ui-exercises', {});
      rec[hashStr(ex.payload)] = true;
      saveJSON('ml-ui-exercises', rec);
      // 卡片回显已通过
      const card = document.querySelector('.ml-exercise.is-active-judging');
      if (card) card.classList.add('is-passed');
      document.querySelectorAll('.ml-exercise').forEach((c) => {
        const b = c.querySelector('.ml-runbtn');
        if (b && b.dataset.mlpyex && loadJSON('ml-ui-exercises', {})[hashStr(b.dataset.mlpyex)]) {
          c.classList.add('is-passed');
          if (!c.querySelector('.pass-mark')) {
            const head = c.querySelector('.ml-card-head');
            if (head) head.appendChild(el('span', 'pass-mark', '✓ 已通过'));
          }
        }
      });
    } else {
      this.status('输出未匹配 —— 对照下方逐行比对调整', 'is-err');
    }
  }

  renderDiff(rows) {
    this.$diff.textContent = '';
    this.$diff.appendChild(el('div', '', '── 期望 vs 实际 ──────────────'));
    rows.forEach((r, i) => {
      const line = el('div', 'pd-row');
      const mark = el('span', r.ok ? 'pd-ok' : 'pd-bad', r.ok ? '✓' : '✗');
      const g = el('span', r.ok ? 'pd-ok' : 'pd-bad', `实际[${i}] ${r.g}`);
      const w = el('span', 'pd-exp', `期望[${i}] ${r.w}`);
      line.append(mark, g, w);
...
/* 卡片渲染后回显已通过状态(观察 DOM,read 页正文注入后触发) */
function echoPassed() {
  const rec = loadJSON('ml-ui-exercises', {});
