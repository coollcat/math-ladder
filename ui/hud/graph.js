/* ============================================================
   HUD 皮肤 · 课级知识树画布(自包含模块)
   用法: const t = mountTree(container, { flat, edges, onOpen });
         t.focus(id) / t.clear() / t.fitAll() / t.destroy()
...
export function mountTree(container, { flat, edges, onOpen }) {
  ...
  const up = flat.map(() => []);    // up[i] = i 的先修下标
  const down = flat.map(() => []);  // down[j] = 依赖 j 的下标
  ...
  function layoutOf(ids) {
    ...
    for (const [l, arr] of groups) {
      ...
      arr.forEach((idx, k) => out.set(idx, { x: k * COL_W - w / 2, y: TOP_PAD + l * LEVEL_H }));
    }
    return out;
  }
  ...
  toolbar.innerHTML = `
    <button type="button" class="hud-btn" data-act="fit"><span>⤢ 全览</span></button>
    <button type="button" class="hud-btn" data-act="clear"><span>✕ 清除聚焦</span></button>
    <button type="button" class="hud-btn is-amber" data-act="grow"><span>↻ 重播生长</span></button>`;
  ...
  vp.addEventListener('pointermove', (e) => {
    if (drag) { tx = drag.tx + e.clientX - drag.x; ty = drag.ty + e.clientY - drag.y; apply(); }
      ...
      const d2 = Math.hypot(pts2[0].x - pts2[1].x, pts2[0].y - pts2[1].y);
      if (pinch.baseD > 0) zoomAt((pts2[0].x + pts2[1].x) / 2, (pts2[0].y + pts2[1].y) / 2, pinch.baseK * (d2 / d === Infinity ? 1 : (d2 / pinch.lastD || 1)));
      pinch.lastD = d2;
    }
  });
  const endDrag = (e) => { drag = null; pinch && pinch.delete(e.pointerId); vp.style.cursor = 'grab'; };
  ...
  vp.addEventListener('pointerdown', (e) => {
    if (e.isPrimary && e.pointerType === 'touch') {
      ...
      if (pinch.size === 2) { const pts = [...pinch.values()]; pinch.baseD = Math.hypot(pts[0].x - pts[1].x, pts[0].y - pts[1].y); pinch.baseK = k; pinch.lastD = pinch.baseD; drag = null; }
    }
  });
  ...
  function focus(idx) {
    ...
    for (let i = 0; i < nodeEls.length; i++) {
      if (!vis.includes(i)) nodeEls[i].style.opacity = '';
    }
    apply();
    fitAll();
    showInfo(idx);
